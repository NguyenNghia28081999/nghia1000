...
namespace WebsiteBanGiay.Areas.Admin.Controllers
{
    public class GiaysController : Controller
    {
        ...
        public ActionResult Index(int? page)
        {
            int num = 15;
            int pagenum = (page ?? 1);
            var giays = db.Giays.Include(g => g.DanhMuc);
        ...
        public ActionResult Create(Giay giay, HttpPostedFileBase fileimg)
        {
            if (ModelState.IsValid)
            {
                var img = Path.GetFileName(fileimg.FileName);
                var pathimg = Path.Combine(Server.MapPath("~/HinhAnh/Giay"), img);
                if (fileimg == null)
                {
                ...
                giay.HinhAnhGiay = fileimg.FileName;
                giay.NgayHangVe = DateTime.Now;
                db.Giays.Add(giay);
                db.SaveChanges();
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit(Giay giay, HttpPostedFileBase fileimg)
        {
            if (ModelState.IsValid)
            {
                var img = Path.GetFileName(fileimg.FileName);
